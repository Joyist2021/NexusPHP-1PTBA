...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class)
	...
	if (!isset($_GET["cmtpage"])) {
		...
		tr($lang_details['torrent_dl_url'],sprintf('<style>#torrent_dl_url{color:#ff0000}</style><b title="%s" id=torrent_dl_url><a title="为了保护你的passkey，链接已隐藏，右键看查" href="%s%s/download.php?id=%u&passkey=%s">[鼠标点击复制PassKey链地址]</a>，[您的个人密钥PassKey，请勿泄露！复制此链接可在BT客户端使用下载种子]。</b>',$lang_details['torrent_dl_url_notice'],get_protocol_prefix(),$BASEURL,$id,$CURUSER['passkey']), 1);
